...
public class JobCompletionListener implements JobExecutionListener {
	...
	public void afterJob(JobExecution jobExecution) {
		...
		jobExecution.getStepExecutions().stream().forEach(e -> {
			log.info("{} status: {}", e.getStepName(), e.getExitStatus());
		});
		
		if (ExitStatus.COMPLETED.equals(jobExecution.getExitStatus())
				&& jobExecution.getStepExecutions().stream().filter(e -> !"eventGenerationDeciderStep".equals(e.getStepName())).noneMatch(e -> e.getExitStatus() == ExitStatus.FAILED)) {
